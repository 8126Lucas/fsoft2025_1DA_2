...
/*
TEST_F(DriverContainerTest, InvalidDriver) {
    char license = 'B';
    Driver *driver = new Driver(-1, license, 15);
    testing::internal::CaptureStdout();
    driverContainer->add(*driver);
    string output = testing::internal::GetCapturedStdout();
    EXPECT_EQ(output, "!! Error: \"Driver data is invalid\" !!\n");
...
/*
TEST_F(DriverContainerTest, RemoveInvalidDriverID) {
    char license = 'B';
    Driver *driver = new Driver(1, license, 30);
    driverContainer->add(*driver);
    ...
    driverContainer->remove(-1);
    string output = testing::internal::GetCapturedStdout();
    EXPECT_EQ(output, "!! Error: \"Driver (id): -1\" is invalid !!\n");
    EXPECT_EQ(driverContainer->listDrivers().size(), 1);
    delete driver;
}*/
...
TEST_F(DriverContainerTest, AddVacationToDriver) {
    ...
    EXPECT_EQ(driver->getVacation(), vacation);
...
/*
TEST_F(DriverContainerTest, AddInvalidVacationID) {
    char license = 'B';
    Driver *driver = new Driver(1, license, 30);
    driverContainer->add(*driver);
    ...
    driverContainer->addVacation(vacation);
    string output = testing::internal::GetCapturedStdout();
    EXPECT_EQ(output, "!! Error: \"Vacation (id): -1\" is invalid !!\n");
    EXPECT_EQ(driverContainer->get(1)->getVacations().size(), 0);
